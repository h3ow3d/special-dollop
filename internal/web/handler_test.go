...
func TestAssessmentsProtectedWithoutAuth(t *testing.T) {
	h, _ := newTestHandler(t)
	r := httptest.NewRequest(http.MethodGet, "/assessments", nil)
	w := httptest.NewRecorder()
	h.Router([]byte("12345678901234567890123456789012")).ServeHTTP(w, r)
	if w.Code != http.StatusFound {
		t.Fatalf("expected redirect for unauthenticated assessments, got %d", w.Code)
	}
	if loc := w.Header().Get("Location"); loc != "/" {
		t.Fatalf("expected redirect to / got %s", loc)
	}
